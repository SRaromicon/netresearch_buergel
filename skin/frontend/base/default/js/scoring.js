    ...
    nextStep: function(transport){
        if (transport && transport.responseText) {
            ...
            if (response.success) {
                ...
                if (response.failing_field) {
                    $$('.checkout-'+response.failing_field).invoke('addClassName', response.failing_field+'-failed');
                    new Effect.ScrollTo($$('.checkout-'+response.failing_field)[0], 0.5);
                }
                var msg = response.error_messages;
                if(!response.goto_section && $('review_messagebox_bottom')) {
                    if (typeof(msg)=='object') {
                        msg = msg.join("</li><li>");
                    }
                    $$('#review_messagebox_bottom .error-msg')[0].innerHTML = "<ul><li>" + msg + "</li></ul>";
                    $('review_messagebox_bottom').show();
                    new Effect.ScrollTo($('review_messagebox_bottom'), 0.5);
                } else {
                    if (typeof(msg)=='object') {
                        msg = msg.join("\n");
                    }
                    //alert(msg);
                    try {
                        // Prevent js-errors on quest-checkout
                        $('billing-address-select').options[0].disabled = 'disabled';
                        $('billing-address-select').options[$('billing-address-select').options.length-1].selected = 'selected';
